"""This file work like builder. All blocks were declareted in utils.eff_net_utils"""
...
from utils import eff_net_utils as efficientnet_model
import utils
...
def efficientnet(width_coefficient=None,
  ...
  """it seems like r - repeat, k - kernel_size. s- ?(mb stride), eN - N == number MBConvN(look at paper arch table), i - input chanel, o - output chanel """
  blocks_args = [
...
def build_model_base(images, model_name, training, override_params=None, bottleneck_layer_size=128, use_Gdw=True):
  ...
  Args:
    ...
    override_params: A dictionary of params for overriding. Fields must exist in
      efficientnet_model.GlobalParams.
    bottleneck_layer_size: size of embeddings vector
    use_Gdw: use global depthwise conv or no
  Returns:
  ...
  with tf.variable_scope(model_name):
    model = efficientnet_model.Model(blocks_args, global_params)
    net = model(images, training=training, features_only=True)

    with tf.variable_scope('Logits'):
        if use_Gdw:
            #Global Depthwise Conv transform from [k, k, c] to [1, 1, c] if u use kernel [k, k]
            net = slim.separable_conv2d(inputs=net, num_outputs=None, kernel_size=[7, 7], stride=1,
                                        depth_multiplier=1.0, activation_fn=None, padding='VALID')
            net = slim.conv2d(inputs=net, num_outputs=320, kernel_size=[1, 1], stride=1, activation_fn=None, padding='VALID')

            model.end_points['GDConv'] = net

        logits = slim.conv2d(net, bottleneck_layer_size, kernel_size=[1, 1], stride=1, activation_fn=None, scope='LinearConv1x1')
        model.end_points['Logits'] = logits
